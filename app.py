...
# 3. FONDO Y ESTILO (ETIQUETAS VISIBLES Y FORMULARIOS CLAROS)
def agregar_fondo(nombre_archivo):
    try:
        ...
        st.markdown(f"""
            ...
            /* IDENTIFICADORES DE CAMPO EN NEGRO Y NEGRITA */
            label, .stMarkdown p, .stSelectbox label, .stDateInput label, .stTextInput label, .stNumberInput label {{
...
if st.session_state.sesion is None:
    ...
    with col_l2:
        if not st.session_state.recuperar:
            ...
            preg = st.text_input("Seguridad: ¿Nombre de la congregación?")
...
if rol in ["finanzas", "todos"]:
    with tabs[idx]:
        ...
        if t_auto: t_final = st.number_input("Tasa BCV Detectada", value=t_auto)
        else: t_final = st.number_input("Tasa BCV Manual", min_value=1.0, value=55.0)
        
        with st.form("f_fin", clear_on_submit=True):
            n_f = st.text_input("Nombre del Hermano/a")
            r_f = st.text_input("Referencia (Pago Móvil / Efectivo)")
            ...
            if st.form_submit_button("💾 Guardar"):
                ...
                conn.commit(); st.success(f"Registrado: {m_usd:.2f} USD")
    idx += 1
    with tabs[idx]:
        df_f = pd.read_sql_query("SELECT * FROM finanzas", conn)
        st.dataframe(df_f)
        if st.button("📱 WhatsApp Informe"):
            msg = urllib.parse.quote(f"📊 Informe Tesorería: {df_f['monto_usd'].sum():.2f} USD")
...
if rol in ["materiales", "todos"]:
    with tabs[idx]:
        with st.form("f_mat", clear_on_submit=True):
            ...
            desc_m = st.text_area("Descripción de la Donación")
...
if rol in ["asistencia", "todos"]:
    with tabs[idx]:
        ...
        with st.form("f_mie", clear_on_submit=True):
            nm = st.text_input("Nombre Completo del Hermano/a")
            tm = st.text_input("Teléfono (WhatsApp)")
            dm = st.text_input("Dirección de Habitación")
    ...
    with tabs[idx]:
        ...
        if not df_l.empty:
            with st.form("f_as_c"):
                herm = st.selectbox("Seleccione al Miembro", df_l['nombre'].tolist())
                ...
                if st.form_submit_button("✅ Marcar"):
                    ...
                    conn.commit(); st.success("Marcado")
    idx += 1
    with tabs[idx]:
        st.subheader("📝 Reporte Asistencia")
        try:
...
if rol == "todos":
    with tabs[idx]:
        ...
        c3.metric("Donaciones Mat.", len(pd.read_sql_query("SELECT * FROM materiales", conn)))
    idx += 1
    with tabs[idx]:
        t_sel = st.selectbox("Ver Base de Datos Completa", ["miembros", "finanzas", "materiales", "asistencia", "eventos"])
